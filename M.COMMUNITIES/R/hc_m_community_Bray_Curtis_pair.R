...
#' hc_m_community_Bray_Curtis_pair(entity,minx=5,miny=5,maxx=45,maxy=45,scale=5,edge="centroid")
hc_m_community_Bray_Curtis_pair=
function(entity,minx,miny,maxx,maxy,scale,edge)
{
  ...
  m_comm_pair=beta.pair.abund(abundance, index.family="bray")

  bray.bal=m_comm_pair$beta.bray.bal
  bray.bal=round(as.matrix(bray.bal),3)
  bray.gra=m_comm_pair$beta.bray.gra
  bray.gra=round(as.matrix(bray.gra),3)
  bray=m_comm_pair$beta.bray
  bray=round(as.matrix(bray),3)
  diag(bray.bal)=NA
  diag(bray.gra)=NA
  diag(bray)=NA
